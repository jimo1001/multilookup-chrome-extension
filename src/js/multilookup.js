...
Result.prototype.createElement = function() {
    ...
    this.element = $n('div', { "class": "MLu_result " + SITEINFO[this.infoId]["type"] }, void(0));
    var link = $n('a', { href: this.url, target: "_blank" }, SITEINFO[this.infoId]["name"]);
    var icon = (SITEINFO[this.infoId]["icon"]) ? $n('img', { src: SITEINFO[this.infoId]["icon"] }, void(0)) : null;
    var title = $n('div', { "class": "MLu_result_title" }, icon ? [icon, link] : [link]);
    var article = $n('div', { "class": "MLu_result_article" }, void(0));
...
ResultGroup.prototype.createCellElement = function() {
    var table = $n('div', { "class": "MLu_table" }, void(0));
...
ResultGroup.prototype.addMessage = function(message, type) {
    type = type || "notice";
    var msg = $n("span", { "class": "MLu_message_" + type }, void(0));
...
var MultiLookup = {
    ...
    initElement: function() {
        ...
        if (!this.node) {
            this.node = $n('div', {id:"MultiLookup"}, void(0));
    ...
    onMouseup: function(evt) {
        ...
        if (enable_context_menus || (enable_context_menus === void(0))) {
            if (evt["button"] === 0)
                MultiLookup.postMessage({id: "contextmenu-update", value: text});
        }
    ...
    onMessage: function(msg) {
        ...
        switch (id) {
            ...
            case "config":
                if (value)
                    CONFIG = value;
                MultiLookup.loadedConfigCallback.call(this, value);
                break;
            case "lookup":
                if (value) {
...
var keybinds = {
    ...
    Keybind: function(element, key, callback, force) {
        ...
        this.execute = function(evt) {
            if (!this.force) {
                if (keybinds.isInputable(evt.target)) {
                    return;
                }
            }
    ...
    getKeybinds: function(element, key) {
        ...
        if ((!element && !key) || (!kbs || kbs.length < 1)) {
            return kbs;
        }
