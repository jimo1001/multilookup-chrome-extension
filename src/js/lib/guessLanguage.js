...
(function(global, undefined) {
  ...
  var guessLanguage = function() {
      ...
      function identify(text, callback) {
        ...
        _score = scripts["Hangul Syllables"] + scripts["Hangul Jamo"] + scripts["Hangul Compatibility Jamo"];
        if (_score >= 0.4) {
          //callback.apply(undefined, ["ko"]);
          //return;
          langs.push(["ko", _score]);
        }

        _score = scripts["Greek and Coptic"];
        if (_score >= 0.4) {
          //callback.apply(undefined, ["el"]);
          //return;
          langs.push(["el", _score]);
        }

        _score = scripts["CJK Unified Ideographs"] * 0.5 + (scripts["Hiragana"] + scripts["Katakana"] + scripts["Katakana Phonetic Extensions"]) * 2;
        if (_score >= 0.5) {
          //callback.apply(undefined, ["ja"]);
          //return;
          langs.push(["ja", _score]);
        }

        _score = scripts["CJK Unified Ideographs"] * 0.5 + (scripts["Bopomofo"] + scripts["Bopomofo Extended"] + scripts["KangXi Radicals"]) * 2;
        if (_score >= 0.4) {
          //callback.apply(undefined, ["zh"]);
          //return;
          langs.push(["zh", _score]);
        }
        if (langs.length > 0) {
          callback.apply(undefined, [sortLangs(langs)]);
          return;
        }
        ...
        for (var i = 0, l = SINGLETONS.length; i < l; i++) {
          if (scripts[SINGLETONS[i][0]] >= 0.4) {
            langs.push([SINGLETONS[i][1], SINGLETONS[i][0]]);
            return;
          }
        }
        if (langs.length > 0) {
        ...
        if (scripts["Latin-1 Supplement"] + scripts["Latin Extended-A"] + scripts["IPA Extensions"] >= 0.4) {
          check(text, EXTENDED_LATIN, function(latin_lang) {
            if (latin_lang == "pt") {
              ...
              callback.apply(undefined, [[latin_lang, 1]]);
            }
          });
          return;
        }
        ...
        callback.apply(undefined, [[UNKNOWN, 0]]);
        // return;
      }
      ...
      function check(sample, langs, callback) {
        ...
        var _langs = [], highest = 0;
        for (var i = 0, len = sortedScores.length; i < len; i++) {
          //maximum_score = maximum_score > 8000 ? maximum_score : 8000;
          var lang = sortedScores[i][0];
          var score = (maximum_score - sortedScores[i][1]) / (maximum_score > 5000 ? maximum_score : 5000);
          if (i == 0) {
            highest = score;
          }
          if (highest < 0.25 && lang == "en") {
            score = score + 0.25;
          }
          if (score > 0) {
            _langs.push([lang, score]);
          }
        }
        _langs = _langs.sort(function(a, b) {
          return b[1] - a[1];
        });
        // return the best match we've now calculated
        callback.apply(undefined, [_langs]);
        //return;
      }
